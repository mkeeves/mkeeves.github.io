...
(function() {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const postsGrid = document.querySelectorAll<HTMLElement>('.posts-grid');
        ...
        function postMatchesQuery(postCard: HTMLElement, query: string): boolean {
            const normalizedQuery = normalizeText(query);
            
            // Get post title
            const titleElement = postCard.querySelector<HTMLElement>('h3 a, h4 a');
            const title = titleElement ? titleElement.textContent || '' : '';
            
            // Get post excerpt/content
            const excerptElement = postCard.querySelector<HTMLElement>('.post-excerpt, p');
            const excerpt = excerptElement ? excerptElement.textContent || '' : '';
            
            // Get post meta (date, category)
            const metaElement = postCard.querySelector<HTMLElement>('.post-meta');
            const meta = metaElement ? metaElement.textContent || '' : '';
            
            // Get tags
            const tagElements = postCard.querySelectorAll<HTMLElement>('.tag');
        ...
        function performSearch(query: string | null | undefined): void {
            ...
            if (!trimmedQuery) {
                ...
                const sectionHeaders = document.querySelectorAll<HTMLElement>('.section h2, .section h3');
            ...
            const heroSection = mainContent.querySelector<HTMLElement>('.hero');
            if (heroSection) {
        ...
        function initializeSearch(): void {
            const currentSearchInput = document.querySelector<HTMLInputElement>('.search-input');
